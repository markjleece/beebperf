...
namespace BeebPerf.ux
{
    internal class MemoryView : DataGridView
    {
        ...
        private void SelectionChangedFunc(object? sender, EventArgs e)
        {
            ...
            {
                _SelectedMemoryAccess = _MemoryAccesses[SelectedRows[0].Index];
                // TODO: refresh memoryRoutinesView
            }
            else
            {
                _SelectedMemoryAccess = null;
        ...
        protected override void OnColumnHeaderMouseClick(DataGridViewCellMouseEventArgs e)
        {
            ...
            switch (e.ColumnIndex)
            {
                ...
                case ReadWriteCountColumnIndex:
                    if (ascending)
                        _MemoryAccesses.Sort((a, b) => a.ReadWriteCount.CompareTo(b.ReadWriteCount));
                    else
                        _MemoryAccesses.Sort((a, b) => b.ReadWriteCount.CompareTo(a.ReadWriteCount));
                    break;
        ...
        public void SetMemoryAccesses(List<MemoryAccess> memoryAccesses)
        {
            ...
            _TotalReadWriteCount = totalReadCount + totalWriteCount;
        ...
        private void CellValueNeededFunc(object? sender, DataGridViewCellValueEventArgs e)
        {
            ...
            switch (e.ColumnIndex)
            {
                ...
                case ReadWriteCountColumnIndex:
                    e.Value = FormatCount(memoryAccess.ReadWriteCount, _TotalReadWriteCount);
                    break;
        ...
        private void SortCompareFunc(object? sender, DataGridViewSortCompareEventArgs e)
        {
            ...
            switch (e.Column.Index)
            {
                ...
                case ReadWriteCountColumnIndex:
                    e.SortResult = a.ReadWriteCount - b.ReadWriteCount;
                    e.Handled = true;
                    break;
        ...
        public class CellTemplate : DataGridViewTextBoxCell
        {
                ...
                DataGridViewElementStates cellState)
            {
                ...
                {
                    ReadWriteCountColumnIndex => (double)memoryAccess.ReadWriteCount / memoryView._TotalReadWriteCount,
        ...
        private int _TotalReadWriteCount;
